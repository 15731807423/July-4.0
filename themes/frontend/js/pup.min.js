!function(window, factory) {
    if (document.addEventListener) {
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            forms.length && forms.forEach((function(form) {
                pup.init(form)
            }
            ))
        }
    ...
    function completeURL(url) {
        return "/" === url.slice(-1) && (url += "index.html"),
        url.toLowerCase()
    }
    function getNotebook() {
        ...
        try {
            if (!notebook)
                throw "notebook is not available";
            if (!(notebook = JSON.parse(notebook)) || "0.3.0" !== notebook.version)
                throw "notebook is not available or version does not match";
            var len = notebook.memory.length
              , last = notebook.memory[len - 1];
            if (now - last.leave > 5e3)
                throw ">=5s";
            if (last.url === refer.url)
                ...
                if (last.url !== current.url)
                    throw "other";
                (current = last).leave = now
            }
        } catch (e) {
            notebook = {
                version: "0.3.0",
                memory: [refer, current]
            }
        }
    }
    function takenote() {
    ...
    function hardworking() {
        ...
        takenote(),
        _ticktack = setTimeout(hardworking, 3e3)
    }
    return {
        ...
        notes: function() {
            ...
            for (var notes = {
                ...
                trace: []
            }, record = null, i = notebook.memory.length - 1; i > 0; i--)
                record = notebook.memory[i],
                notes.trace.push([record.url, Math.round((record.leave - record.enter) / 1e3)]);
...
document.addEventListener('DOMContentLoaded', function() {
    var xform = document.querySelectorAll('input[name="form_required"]');
    var iform;
    for (iform = 0; iform < xform.length; iform++) {
        xform[iform].value = "form_required";
    }
});
