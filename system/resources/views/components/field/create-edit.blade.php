...
<el-form ref="field_{{ $mode }}_form" :model="{{ $model }}" label-width="108px">
    ...
    {{-- 字段类型 --}}
    @if ($mode === 'creation')
    <el-form-item label="字段类型" prop="field_type" size="small" class="has-helptext" :rules="[{required:true,message:'『字段类型』不能为空',trigger:'submit'}]">
    ...
    <el-form-item label="帮助" prop="helptext" size="small" class="has-helptext">
        <el-input v-model="{{ $model }}.helptext" type="textarea" rows="3"></el-input>
    ...
    <el-form-item label="搜索权重" size="small" class="has-helptext">
        <el-input-number v-model="{{ $model }}.weight" size="small" controls-position="right" :min="0" :step="1" :precision="0"></el-input-number>
    ...
    <el-form-item label="字数" size="small" class="has-helptext">
        <el-input-number v-model="{{ $model }}.maxlength" size="small" controls-position="right" :min="0"></el-input-number>
    ...
    {{-- 引用范围 --}}
    <el-form-item v-if="fieldMetakeys.indexOf('reference_scope') >= 0" label="引用范围" size="small" :rules="[{required:true,message:'『引用范围』不能为空',trigger:'submit'}]" class="has-helptext">
        @if ($mode == 'creation')
        <el-cascader size="small" placeholder="--选择引用范围--" clearable v-model="{{ $model }}.reference_scope" :options="referenceScope" @hook:created="initReferenceScope"></el-cascader>
