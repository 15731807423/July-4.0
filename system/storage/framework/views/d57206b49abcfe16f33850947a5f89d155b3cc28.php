<?php $__env->startSection('h1', '文件管理'); ?>
  ...
  let app = new Vue({
    ...
    computed: {
      allowedMimeTypes() {
        ...
        if (category === 'files') {
          return '<?php echo e(implode(",", config("media.categories.files.valid_mime"))); ?>';
        }

        if (category === 'images') {
          return '<?php echo e(implode(",", config("media.categories.images.valid_mime"))); ?>';
        }
      },
    },
    ...
    methods: {
      ...
      load(path) {
        ...
        // 后台获取目录内容
        axios.post("<?php echo e(short_url('media.under')); ?>", {
      ...
      renameFile(file, event) {
        ...
        this.promptFilename('', '文件名：', fileName).then(({ value }) => {
          ...
          this.process({
            loading: '正在修改文件名 ...',
            action: "<?php echo e(short_url('media.rename', 'files')); ?>",
            data: {
              cwd: path,
              old_name: file.name,
              new_name: value + ext,
      ...
      deleteFile(file, event) {
        ...
        .then(() => {
          this.process({
            action: "<?php echo e(short_url('media.delete', 'files')); ?>",
            loading: '正在删除文件 ...',
            data: {
              cwd: path,
              name: file.name,
      ...
      createFolder() {
        ...
        .then(({ value }) => {
          this.process({
            action: "<?php echo e(short_url('media.mkdir')); ?>",
            loading: '正在创建文件夹 ...',
            data: {
              cwd: path,
              name: value,
...
<?php echo $__env->make('layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\laragon\www\test\system\resources\views/media/index.blade.php ENDPATH**/ ?>