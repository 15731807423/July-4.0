            ...
            <div class="el-form-item__content">
                <div class="jc-embeded-field__header">
                    ...
                    <div class="jc-embeded-field__buttons">
                        <button type="button" title="新建字段" class="md-button md-icon-button md-dense md-accent md-theme-default" @click.stop="createField()">
                ...
                <div class="jc-table-wrapper">
                    <table class="jc-table jc-dense is-draggable with-drag-handle with-operators">
                        ...
                        <tbody is="draggable" v-model="fields" :animation="150" ghost-class="jc-drag-ghost" handle=".jc-drag-handle" tag="tbody">
                            <tr v-for="field in fields" :key="field.field_id">
                                ...
                                <td>
                                    <div class="jc-operators">
                                        <button type="button" class="md-button md-icon-button md-primary md-theme-default" title="编辑" @click.stop="editField(field)">
                                            <svg class="md-icon jc-svg-icon"><use xlink:href="#jcon_edit_circle"></use></svg>
                                        </button>
                                        <button type="button" class="md-button md-icon-button md-accent md-theme-default" title="删除" @click.stop="removeField(field)">
...
<el-dialog id="field_editor" title="新建/编辑规格字段" top="-5vh" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="fieldEditorVisible">
    <el-form ref="field_creat_edit_form" :model="currentField" :rules="fieldRules" label-width="108px">
        <el-form-item label="ID" prop="field_id" size="small" class="has-helptext">
            <el-input v-model="currentField.field_id" name="field_id" native-size="60" maxlength="32" show-word-limit :disabled="!isEditable"></el-input>
        ...
        <el-form-item label="字段类型" prop="field_type_id" size="small" class="has-helptext">
            <el-select v-model="currentField.field_type_id" placeholder="--选择字段类型--" :disabled="!isEditable || fieldEditing">
                <el-option v-for="type in fieldTypes" :key="type.id" :label="type.label" :value="type.id">
                </el-option>
            </el-select>
            <span class="jc-form-item-help">@{{ fieldTypeHelp }}</span>
        </el-form-item>
        <el-form-item label="小数位" prop="places" size="small" class="has-helptext" v-if="currentField.field_type_id=='float'">
            <el-input-number v-model="currentField.places" size="small" controls-position="right" :min="0" :max="6" :disabled="!isEditable"></el-input-number>
        ...
        <el-form-item label="可筛选" size="small">
        ...
        <template v-if="currentField.is_groupable">
            <el-form-item label="筛选类型" size="small">
                <el-radio-group v-model="currentField.screen_type">
                    ...
                    <el-radio :label="5">下拉菜单</el-radio>
                </el-radio-group>
            </el-form-item>

            <el-form-item label="筛选默认值" size="small">
                <el-input v-model="currentField.screen_default" native-size="60"></el-input>
