...
<el-dialog id="field_selector" top="-5vh" :show-close="false" :visible.sync="tabs.visible" @open="syncSelectedToSelection" class="jc-dialog-form">
    <el-tabs v-model="tabs.current" type="card" class="jc-tabs-mini">
        <el-tab-pane label="选择字段" name="selection" class="md-scrollbar md-theme-default">
            <el-table ref="optional_fields_table" :data="selectionData.fields" style="width: 100%;" class="jc-table jc-dense" @selection-change="handleSelectionChange" @hook:mounted="syncSelectedToSelection">
    ...
    let app = new Vue({
        ...
        methods: {
            ...
            initReferenceScope() {
                ...
                axios.get("{{ short_url('resources.entity_types') }}").then((response) => {
            ...
            submit() {
                ...
                this.$refs.main_form.validate().then(() => {
                    ...
                    axios.{{ $context['mode']==='edit' ? 'put' : 'post' }}(action, data).then((response) => {
                        window.location.href = "{{ short_url('node_types.index') }}";
                    }).catch((error) => {
                        loading.close();
                        console.error(error);
