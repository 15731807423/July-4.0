    ...
    >
        <div class="jc-tree-node-inner" slot-scope="{ node, data }" @dblclick.stop="changeRoot(data)">
            ...
            <button
                ...
                type="button"
                title="切换位置"
                class="md-button md-mini md-primary md-theme-default"
                @click.stop="moveRecycle(node)"
            >
                <div class="md-ripple">
                    <div class="md-button-content">
                        <i class="md-icon md-icon-font md-theme-default">push_pin</i>
                    </div>
                </div>
            </button>
        </div>
    ...
    let app = new Vue({
        ...
        methods: {
            ...
            getNodeClass(node, tree) {
                return node.data.id === 'recycle' ? ['catalog-tree__recycle', this.recycle.place] : '';
            },
            ...
            // 获取节点数据对应的树节点
            getTreeNode(node) {
                return this.$refs.catalog_tree.getNode(_nodes[node.id]);
            },
            ...
            getParentNode(node) {
                ...
                return treeNode.parent.level === 0 ? this.root : _nodes[treeNode.parent.data.id];
            },
            ...
            // 移除节点（从树里或回收站里）
            removeNode(node) {
                node.id = isNaN(parseInt(node.id)) ? node.id : parseInt(node.id);
            ...
            transferNodeToRecycle(parent, nodeInTree) {
                ...
                const recycle = this.getRecycle(), nodeInRecycle = {
                    id: nodeInTree.id,
            ...
            getNodeAttr(id, attr) {
                if (id === 'recycle') return attr === 'title' ? '节点回收站' : '(未定义)';

                const info = _details[id];

                if (!info || !info[attr]) return '(已删除)';

                if (attr == 'updated_at' || attr == 'created_at') {
                    return moment(info[attr]).fromNow().replace('minutes', 'm').replace('seconds', 's');
                }
            ...
            initRecycle() {
                // 添加回收站
                const recycle = _nodes.recycle, index = this.tree.indexOf(recycle);
            ...
            sortRecycle() {
                ...
                if (this.recycle.orderBy === 'default') {
                    recycle.children.splice(0, recycle.children.length, ...recycle.children);
                    return;
                }

                const params = this.recycle.orderBy.split(','),
                    attr = params[0],
                    asc = params[1] === 'asc' ? 1 : -1,
                    children = _.cloneDeep(recycle.children);

                children.sort((a, b) => {
                    return _details[a.id][attr] > _details[b.id][attr] ? asc : -asc;
                });
            ...
            moveRecycle(recycleNode) {
                const places = ['top', 'center', 'bottom'],
                    newPlace = places[(places.indexOf(this.recycle.place) + 1)%3],
                    el = recycleNode.treeNode.$el;
            ...
            submit() {
                ...
                const data = { 'positions': toPositions(tree) };

                axios.put("{{ short_url('catalogs.sort', $model['id']) }}", data).then((response) => {
                    window.location.href = "{{ short_url('catalogs.index') }}";
                }).catch((error) => {
