...
@section('h1')
{{ __('backend.'.$context['mode']) }}表单
@endsection

@section('main_content')
<el-form id="main_form" ref="main_form" :model="mold.model" :rules="mold.rules" label-position="top">
    <div id="main_form_left">
        ...
        <el-form-item label="默认主题" prop="subject" size="small" class="has-helptext">
            <el-input v-model="mold.model.subject" name="subject" native-size="100"></el-input>
            ...
            <div class="el-form-item__content">
                <div class="jc-embeded-field__header">
                    ...
                    <div class="jc-embeded-field__buttons">
                        <button type="button" title="选择或新建字段" class="md-button md-icon-button md-dense md-accent md-theme-default" @click.stop="tabs.visible = true">
                            <div class="md-ripple">
                ...
                <div class="jc-table-wrapper">
                    <table class="jc-table jc-dense is-draggable with-drag-handle with-operators">
                        ...
                        <x-mold.field-list model="mold.fields" :deletable="true" />
                    </table>
                </div>
...
<el-dialog id="field_selector" top="-5vh" :show-close="false" :visible.sync="tabs.visible" @open="syncSelectedToSelection" class="jc-dialog-form">
    <el-tabs v-model="tabs.current" type="card" class="jc-tabs-mini">
        <el-tab-pane label="选择字段" name="selection" class="md-scrollbar md-theme-default">
            <el-table ref="fields_table" :data="selectionData.fields" style="width: 100%;" class="jc-table jc-dense" @selection-change="handleSelectionChange" @hook:mounted="syncSelectedToSelection">
...
<el-dialog id="field_editor" title="编辑字段" top="-3vh" :visible.sync="field.dialogVisible" class="jc-dialog-form">
    <div class="md-scrollbar md-theme-default js-scroll-wrapper" style="max-height:600px; overflow:hidden auto; padding:0 20px">
    ...
    let app = new Vue({
        ...
        methods: {
            ...
            syncSelectedToSelection() {
                const selected = this.mold.fields.map(field => field.id);
                const table = this.$refs.fields_table;
                if (table) {
            ...
            syncSelectionToSelected() {
                const fields = [];
                const current = this.mold.fields.map(field => field.id);
                const selected = this.selectionData.selection.map(field => field.id);

                // 移除未选中的
                this.mold.fields.forEach(field => {
                ...
                // 更新视图
                this.$set(this.$data.mold, 'fields', fields);
            ...
            createField() {
                ...
                form.validate().then(() => {
                    ...
                    axios.post("{{ short_url('message_fields.store') }}", field).then((response) => {
            ...
            submit() {
                ...
                if (_.isEqual(this.original_mold, this.mold)) {
                    window.location.href = "{{ short_url('message_forms.index') }}";
                    return;
                }
                @endif
                ...
                this.$refs.main_form.validate().then(() => {
                    ...
                    @if ($context['mode'] === 'edit')
                    const action = "{{ short_url('message_forms.update', $model['id']) }}";
                    @else
                    const action = "{{ short_url('message_forms.store') }}";
                    @endif

                    const data = _.cloneDeep(this.mold.model);
                    data.langcode = "{{ $context['content_langcode'] }}";
                    data.fields = this.mold.fields.slice();
