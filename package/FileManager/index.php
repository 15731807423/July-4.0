...
function login_page($is_login_attempt, $sidx, $is_logout, $client_hash) {
    ...
    <script>
        ...
        <?php if($is_login_attempt && $_POST['
        sidx '] !== $sidx) { ?><div class="alert alert-danger" role="alert"><strong>PHP session ID mismatch</strong><br>If the error persists, your PHP is incorrectly creating new session ID for each request.</div><?php } else if($is_login_attempt) { ?>\
        ...
        document.getElementsByTagName('form')[0].addEventListener('submit', function() {
            this.action = '<?php echo isset($_GET['
            logout ']) ? strtok($_SERVER['
            REQUEST_URI '], ' ? ') : $_SERVER['
            REQUEST_URI ']; ?>';
            this.method = 'post';
        }, false);
...
function _basename($path) {
    ...
    // because setlocale(LC_ALL,'en_US.UTF-8')
...
// GET
    ...
    // main document
        ...
        <head>
            ...
            <link href="<?php echo config::$assets ?>css/files.css" rel="stylesheet" a="1">
            <?php get_include('css/custom.css'); ?>
        </head>
        <body class="body-loading"><svg viewBox="0 0 18 18" class="svg-preloader svg-preloader-active preloader-body"><circle cx="9" cy="9" r="8" pathLength="100" class="svg-preloader-circle"></svg>
            ...
            <!-- files -->
            <script src="./npm/node_modules/files.photo.gallery/js/files.js"></script>
        </body>
        </html>
        <?php
    }
