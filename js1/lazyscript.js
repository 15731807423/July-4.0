...
(function (global, undefined) {
    ...
    var findCurrentScript = (function () {
        ...
        // IE10
        if (
            "readyState" in doc.createElement("script") &&
        ...
        return function (scripts) {
            try {
                ...
                if (e.stack) {
                    for (var i = scripts.length - 1; i >= 0; i--) {
                        if (scripts[i].src && e.stack.indexOf(scripts[i].src) > -1)
                            return scripts[i];
                    }
                }
            }
    ...
    var thisScript =
        doc.getElementById("lazyscript") || findCurrentScript(doc.scripts);
    ...
    */
    ...
    function getScriptTask(src) {
        ...
        if (isFunction(src)) {
            ...
            task =
                _cachedTasks[src] ||
                (_cachedTasks[src] = new ScriptTask(src.substring(9), "POLYFILL"));
        } else if (_cachedTasks[src]) {
            task = _cachedTasks[src];
        } else {
            var url = id2Url(src);
            task =
                _cachedTasks[url] || (_cachedTasks[url] = new ScriptTask(url, "FILE"));
