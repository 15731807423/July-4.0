LazyScript.load('jquery', function (global) {
    function Calculation(data) {
        ...
        // 存放所有参数 提交按钮 重置按钮
        const param = {}, submit = data.submit ? $(data.submit) : null, reset = data.reset ? $(data.reset) : null;
        ...
        function refresh() {
            for (let key in param) {
                ...
                if (item.type == 'checkbox') {
                    ...
                    eval(`${key} = ` + ($.type(value) == 'string' ? `"${value}"` : value));
                } else {
        ...
        function checkError() {
            ...
            if (data.gather) {
                ...
                for (let i = 0; i < data.error.length; i++) {
                    let value = eval(data.error[i].name), info = param[data.error[i].name];

                    // 判断文本框的值是否错误
                    if (info.type == 'input') {
                        value !== null && eval(data.error[i].condition) && (
                ...
                for (let i = 0; i < data.error.length; i++) {
                    ...
                    if (info.type == 'input') {
                        ...
                        if (value !== null && eval(data.error[i].condition)) {
        ...
        function checkDisabled() {
            ...
            for (var i = 0; i < data.disabled.length; i++) {
                if (param[data.disabled[i].name].type == 'input') {
                    if (eval(data.disabled[i].condition)) {
        ...
        function gather() {
            ...
            if (Object.keys(errorMessage).length > 0) {
                ...
                eval('(' + data.gather + ')(' + JSON.stringify(errorMessage) + ')');
                data.resultClear && result(null);
                return false;
            } else {
                // 没错误信息则传null 表示没错 用来隐藏元素 返回真表示没错
                eval('(' + data.gather + ')(null)');
                return true;
            }
        }
        ...
        function calculationCheck() {
            ...
            status && (status = eval('!!(' + data.resultCondition + ')'));
