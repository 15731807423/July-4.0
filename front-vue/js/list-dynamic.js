// 函数调用流程
// 执行任何操作后整理好查询条件 调用接口获取对应的数据放在页面里

// 页面加载
// 设置好搜索关键词 筛选 排序 分页的数据 发送请求获取列表数据
...
const dataList = {
    ...
    props: {
        ...
        screenApi: { type: String, required: true },
    ...
    methods: {
        ...
        handleData(index = -1, source = '') {
            ...
            if (this.searchStatus) this.searchList();
        ...
        // 获取当前的筛选列表
        getScreen() {
            axios.post(this.screenApi, { data: this.screenAllSelected })
        },
        ...
        send() {
            ...
            if (this.searchStatus && this.keywords.length > 0) {
                data.search = {
            ...
            if (this.screenStatus) {
                data.screen = {
                    data: this.screenAllSelected,
                    countStatus: this.screenCountStatus,
                    groupCountType: this.screenGroupCountType,
                    nullHidden: this.screenNullHidden
                };
            }
            ...
            data.page = {
                pageSize: this.paginationInside.pageSize,
                currentPage: this.paginationInside.currentPage
            };

            axios.post(this.listApi, data).then(function (data) {
                console.log(data)
            }).catch(function (data) {
                console.log(data)
            });
